...
export default function AgendaPage(){
  ...
  async function loadServices(){ try{ const data = await apiFetch('/api/services', { tenantId }); setSvcs(data); }catch{} }
    ...
    try{
      ...
      await apiFetch('/api/appointments', {
        ...
        init:{ method:'POST', body: JSON.stringify({
          ...
          serviceId: svc,
          employeeId: emp,
          client: { name: cliName, phone: cliPhone },
  ...
  async function load(){
    try{
      ...
      if (filterEmp) qs.set('employeeId', filterEmp);
      if (filterSvc) qs.set('serviceId', filterSvc);
      const data = await apiFetch(`/api/appointments/list?${qs.toString()}`, { tenantId });
      setItems(data.items);
    }catch(e:any){ toast.error(e.message || 'Falha ao carregar'); }
  }
  async function loadEmployees(){
    try{ const data = await apiFetch('/api/employees', { tenantId }); setEmployees(data as Employee[]); } catch {}
  }
  ...
  async function cancel(id: string){
    try{
      await apiFetch('/api/appointments', { tenantId, init: { method: 'PUT', body: JSON.stringify({ id, status: 'CANCELED' }) } });
    ...
    <div className="mx-auto max-w-5xl p-6 space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <CardTitle>Agenda — Filtro e Período</CardTitle>
            ...
            <CardContent>
              {list.length===0 ? (
                ...
                <div className="divide-y border rounded">
                  {list.map(a=>{ const s = new Date(a.startAt), e = new Date(a.endAt); return (
